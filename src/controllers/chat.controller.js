...
import { ALERT, NEW_MESSAGE_ALERT, REFETCH_CHATS, NEW_MESSAGE } from "../../constants/events.js";
import { getOtherMember } from "../../lib/helper.js";

const newGroupChat = TryCatch(async (req, res, next) => {
  console.log("newGroup chat !");
  ...
  emitEvent(req, ALERT, allMembers, `Welcomt to ${name} group `);
...
const getMyGroups = TryCatch(async (req, res, next) => {
  console.log(`Fetching my group data :`);
...
const addMembers = TryCatch(async (req, res, next) => {
  console.log(`Adding new member to the Group `);
  ...
  if (!chat.groupChat)
    return next(new ErrorHandler("This is not a goupChat", 404));
  ...
  const uniqueMembers = allNewMembers
    ...
    .map((i) => i._id);
  // console.log('unu---',uniqueMembers);

  if (uniqueMembers.length === 0)
    return next(
      new ErrorHandler("Memeber is already added to the group", 403)
    );
...
const removeMember = TryCatch(async (req, res, next) => {
  console.log(`Removing the members from the Group `);
  ...
  // console.log(`usersThatWillBeRemoved.name-----`,usersThatWillBeRemoved.name)
  ...
  emitEvent(
    ...
    ALERT,
    chat.members,
    `${usersThatWillBeRemoved.name} has been removed from the group`
  );
